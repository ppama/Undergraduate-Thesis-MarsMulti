...
def plot_n_orbits(rs,labels,cb=pd.mars,show_plot=False,save_plot=False,title='multiple orbits'):
    # 3D plot
...
def coes2rv(coes,deg=False,mu=pd.mars['mu']):
    if deg:
...
def rv2coes(r,v,mu=pd.mars['mu'],degrees=False,print_results=False):
...
def tle2coes(tle_filename,mu=pd.mars['mu']):
    #read tle file
...
def find_rho_z(z,zs=pd.mars['zs'],rhos=pd.mars['rhos']):
